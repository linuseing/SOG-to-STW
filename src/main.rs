...
use sog_to_stw::watchdog::{run_forever, StatusMsg};
use sog_to_stw::Args;
        ...
        |tx| {
            ...
            */
            ...
            let port = args
                .serial_port
                .clone()
                .expect("Please specify a serial port!");

            let baud = args
                .serial_baud
                .clone()
                .expect("Please specify the serial baud rate.");
            ...
            let mut port = match serialport::new(port, baud).open() {
                ...
                Err(_) => {
                    ...
                    tx.send(StatusMsg::Dead)
                        .expect("An unexpected error occurred. Restating Application");
                    return;
                }
            };
                    ...
                    Ok(msg) => match msg {
                        ...
                        Sentence::SOG(sog) => {
                                ...
                                .write_all(vessel_info.to_modified(sog, "xx").as_bytes())
                            {
                                ...
                                Err(err) => {
                                    println!(
                                        "Writing to connection failed {}! Restarting worker!",
                                        err
                                    );
                                    return;
                                }
